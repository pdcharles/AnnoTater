export let SourceDialog = function() {
 ...
 let _SourceDialog= function(ui) {
  ...
  this.node.classList.add('spc_cls_dialog');
  ...
  sep.classList.add('spc_cls_dialog_item','spc_cls_dialog_divider');
  ...
  this.modifications.table = new spc.UI.components.OptionTable(this.ui.app.modsToArray(this.ui.app.definitions.modifications,true),false);
 ...
 _SourceDialog.prototype.update = function() {
  ...
  if (!('name' in this.ui.dialogSession.cache) && aS.sourceDetails.name) {
   this.ui.dialogSession.cache.name = aS.sourceDetails.name;
  }
  ...
  if (!('name' in this.ui.dialogSession.cache)) {
   optionInitialSelection.selected = true;
  }
  this.sourceSelect.append(optionInitialSelection);
  ...
  this.sourceSelect.append(optionSelectNewFiles);

  this.ui.app.sources.forEach((undefined,fileName) => { 
   let option = document.createElement('option');
   option.setAttribute('value', fileName);
   console.log(this.ui.dialogSession.cache.name == fileName);
   if (('name' in this.ui.dialogSession.cache) && this.ui.dialogSession.cache.name == fileName) {
  ...
  if ('name' in this.ui.dialogSession.cache) {
   let source = this.ui.app.sources.get(this.ui.dialogSession.cache.name);
   if (source.fileType == 'mzML' || source.fileType == 'mzXML') {
    if (this.ui.dialogSession.cache.name == aS.sourceDetails.name) {
 ...
 _SourceDialog.prototype.setSelectedSource = function() {
  if (this.sourceSelect.value == '!NEW') {
   ...
   fileSelect.addEventListener('change', (e) => {
    if (fileSelect.files.length) {
     ...
     d.fetchAllScanHeaders().then(() => {
      this.ui.dialogSession.cache.name = f.name;
      this.update();
     })
     spc.UI.removeChildren(this.sourceSelect);
   ...
   this.ui.dialogSession.cache.name = this.sourceSelect.value;
   this.update();
  }
 }

 _SourceDialog.prototype.open = function() {
  let s = this.ui.dialogSession;
  s.open = true;
  s.cache = {};
  this.update();
  s.dialog = this;
